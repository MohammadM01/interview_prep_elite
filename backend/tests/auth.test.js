...
test('Authentication API Test Suite', async (t) => {
  ...
  await t.test('7. authenticated GET /api/auth/me returns current user profile', async () => {
    const res = await request(app)
      .get('/api/auth/me')
  ...
  await t.test('8. logout terminates session and invalidates subsequent /me requests', async () => {
